...
use bool_primitives::Block;
use bool_runtime::RuntimeApi;
use sc_service::{
...
>, ServiceError> {
	...
	let (rpc_extensions_builder, rpc_setup) = {
		...
		let rpc_extensions_builder = move |deny_unsafe, subscription_executor| {
			let deps = bool_rpc::FullDeps {
				client: client.clone(),
				pool: pool.clone(),
				select_chain: select_chain.clone(),
				deny_unsafe,
				babe: bool_rpc::BabeDeps {
					babe_config: babe_config.clone(),
					shared_epoch_changes: shared_epoch_changes.clone(),
					keystore: keystore.clone(),
				},
				grandpa: bool_rpc::GrandpaDeps {
...
pub fn new_light_base(config: Configuration) -> Result<(
	...
	let light_deps = bool_rpc::LightDeps {
	...
	let rpc_extensions = bool_rpc::create_light(light_deps);
...
mod tests {
	...
	use bool_primitives::{Block, DigestItem, Signature};
	use bool_runtime::{BalancesCall, Call, UncheckedExtrinsic, Address};
	use bool_runtime::constants::{currency::CENTS, time::SLOT_DURATION};
	use codec::Encode;
